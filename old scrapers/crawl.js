...
module.exports.crawl_a_story = function crawl_a_story(story_url) {
  ...
  request.get(options, (error, response, html) => {
    if (error) {
      ...
      try {
        story_name = $('div[class="col-12 abp-storypage-headline"]')
          .text()
          .trim();
      ...
      try {
        ...
        img_src = img_div.find('img[class="img-fluid"]').attr("src"); // returns with leading '//'
      } catch (err) {
          ...
          .on("close", () => {
            ...
            $(
              ...
              .each(function (index, element) {
                ...
                if (index != 0) {
                  if (
                    stry_elm.search("feedback@abpdigital.in") == -1 ||
            ...
            story_html += "</div>";
            var out_stream = fs.createWriteStream(
