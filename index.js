	...
	request.get(options, (error, response, html) => {
		if (error) {
			...
			$('article[class="search-result row"]').each((index, element) => {
				const article_link = 'https://www.anandabazar.com' + $(element).find('a[class = "thumbnail"]').attr('href').trim();
...
function crawl_a_story(story_url) {
	...
	request.get(options, (error, response, html) => {
		if (error) {
			...
			const story_name = $('div[class="col-12 abp-storypage-headline"]').text().trim();
			...
			const readme_entry_text = '1. [ ' + readme_entry + ' ](./stories/' + story_name.replace(/ /g, '-') + '.md)\n';
			...
			request(image, options2).pipe(fs.createWriteStream('./metadata/images/' + story_name.replace(/ /g, '-') + '.jpg')).on('close', () => {
				...
				const img_html = '<div align=center> <img src="./../metadata/images/' + story_name.replace(/ /g, '-') + '.jpg" align="center" ></div>\n';

				let story_html = '';
				$('div[class=\'col-12 abp-storypage-articlebody abp-videoarticle-content\']').find('p').each((index, element) => {
					if (index != 0) {
				...
				const out_stream = fs.createWriteStream('./stories/' + story_name.replace(/ /g, '-') + '.md');
				...
				const json_stream = fs.createWriteStream('./metadata/' + story_name.replace(/ /g, '-') + '.json');
